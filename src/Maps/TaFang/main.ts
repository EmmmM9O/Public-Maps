...
interface _TaFang_ {
  ...
  Ai: _Ai_;
  creatTickBlock: <T>(
    init: (bu: _Build_<tickData & T>) => _Build_<tickData & T>,
    data: T
  ) => TickBlock<T>;
...
var TaFang = {
  ...
  creatEffectBlock(tick, effect, time, call) {
    let b = TaFang.creatTickBlock<{}>((bu) => {
      bu.addon.timer.push({
        name: "EffectTick",
        tick: tick,
        run: () => {
        ...
        u.button(
          ...
          (p, v, u) => {
            if (team.money >= b.cost) {
              //UP Block
              let w = (TaFang.Teams[p.team().id].build[v.callback.place] =
                b.creat(
                  v.callback.build.x,
                  v.callback.build.y,
                  p.team(),
      ...
      creat: (
        ...
        for (let i = that.k * -1; i <= that.k; i++) {
          for (let j = that.k * -1; j <= that.k; j++) {
            Vars.world
              .tile(b.x + i, b.y + i)
              .setNet(Blocks.air, Team.get(0), 0);
          }
        }
        Call.effect(Fx.titanSmoke, b.x * 8, b.y * 8, 0, Color.orange);
        let u = UnitTypes.poly.spawn(t, b.x * 8, b.y * 8);
        let run = () => {
          let ai = TaFang.Ai.BuildAi(
            run,
            [
              new BuildPlan(b.x, b.y, 0, that.block),
              new BuildPlan(
                b.x,
                b.y - Math.floor((that.block.size - 1) / 2) - 1,
                1,
                Blocks.itemSource
              ),
            ],
            () => {
              Vars.world
                .tile(b.x, b.y - Math.floor((that.block.size - 1) / 2) - 1)
                .build.configure(that.arrmo);
            }
          );
          u.controller(ai);
          ai.unit = u;
        ...
        updateMovement() {
          try {
            ...
            if (!this.unit.within(x, y, 10)) {
              ...
              this.unit.movePref(
                this.vect.trns(this.unit.angleTo(x, y), this.unit.speed())
              );
            }
          } catch (e) {
            Console.err(e);
  ...
  UnitD(event: _UnitDestroyEvent_) {
    ...
    if (maxt != -1) {
      ...
      if (money <= 0) return;
      Call.label(
        "[#" + Team.get(maxt).color.toString() + "]+" + money,
        0.3,
        unit.x,
        unit.y
      );
...
let drill = (TaFang.Blocks.drill = TaFang.creatDrillBlock(
  Blocks.oreCopper,
  10,
  10
));
...
drill.block=Blocks.mechanicalDrill;
*/
let iceTall = (TaFang.Blocks.IceTall = TaFang.creatEffectBlock(
  3,
  [StatusEffects.freezing, StatusEffects.wet],
  30,
  (bu) => {
...
let flareBlock = (TaFang.Blocks.flareBlock = TaFang.creatUnitBlock(
  UnitTypes.flare,
  10,
  [StatusEffects.overclock, StatusEffects.overdrive],
  10
));
flareBlock.cost = 30;
flareBlock.name = "flare工厂";
...
try {
  Vars.world.tiles.eachTile((t) => {
    if (t.block() == Blocks.message) {
      TaFang.Teams[t.build.team.id].build.push(
        TaFang.Blocks.air.creat(t.x, t.y, t.build.team)
      );
